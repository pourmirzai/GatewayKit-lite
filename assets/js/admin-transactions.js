/**
 * GatewayKit Admin Transactions JavaScript
 *
 * Dedicated handler for the Transactions list table, modals (form data, notes, error info),
 * bulk deletion verification, and clipboard actions.
 *
 * @package GatewayKit
 */
...
(function($) {
    ...
    var AdminTransactions = {
        ...
        bindEvents: function() {
            ...
            // Filter and search buttons: allow native submit
            $(document).on('click', '#gatewaykit-filter-btn, #gatewaykit-search-btn', function() {
                // Let native form submit proceed
            });
            ...
            $(document).on('click', '.gatewaykit-copy-shortcode', function(e) {
                ...
                if (navigator.clipboard && window.isSecureContext) {
                    navigator.clipboard.writeText(textToCopy).then(function() {
                        self.showCopyShortcodeFeedback(button);
                    });
                } else {
        ...
        showCopyShortcodeFeedback: function(button) {
            ...
            icon.removeClass(originalIconClass).addClass('dashicons-yes-alt');
        ...
        formatCurrency: function(amount, currency) {
            currency = currency || gatewaykit_admin_vars.default_currency || 'USD';
            return this.number_format(amount, 2) + ' ' + currency.toUpperCase();
        },
        ...
        showFormDataModal: function(transactionId) {
            ...
            content.html('<p>' + self.escapeHtml(gatewaykit_admin_vars.loading) + '</p>');
            modal.show();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        html += '<div class="gatewaykit-transaction-info">';
                        html += '<h3>' + self.escapeHtml(gatewaykit_admin_vars.transaction_details) + '</h3>';
                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars.id) + ':</strong> ' + self.escapeHtml(data.transaction_id) + '</p>';
                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars.amount) + ':</strong> ' + self.formatCurrency(data.amount, data.currency) + '</p>';
                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars.status) + ':</strong> ' + self.escapeHtml(data.status) + '</p>';
                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars.date) + ':</strong> ' + self.escapeHtml(data.created_at) + '</p>';

                        if (data.user_id) {
                            html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars.user_id) + ':</strong> ' + self.escapeHtml(data.user_id) + '</p>';
                        }

                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars.description) + ':</strong> ' + self.escapeHtml(data.description || '-') + '</p>';
                        html += '</div>';

                        // Form data
                        if (data.form_data && Object.keys(data.form_data).length > 0) {
                            html += '<div class="gatewaykit-form-fields">';
                            html += '<h3>' + self.escapeHtml(gatewaykit_admin_vars.form_data) + '</h3>';
                            ...
                            $.each(data.form_data, function(name, field) {
                                var fieldName = name;
                                var fieldValue = '-';
                ...
                error: function() {
                    content.html('<p>' + self.escapeHtml(gatewaykit_admin_vars.ajax_error_occurred) + '</p>');
                }
            });
        },
        ...
        showNotesModal: function(transactionId) {
            ...
            $('#gatewaykit-notes-content').html('<p>' + self.escapeHtml(gatewaykit_admin_vars.loading) + '</p>');
            modal.show();
        ...
        loadTransactionNotes: function(transactionId) {
            ...
            container.html('<p>' + self.escapeHtml(gatewaykit_admin_vars.loading) + '</p>');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        container.html('<p>' + self.escapeHtml(gatewaykit_admin_vars.error) + '</p>');
                    }
                },
                error: function() {
                    container.html('<p>' + self.escapeHtml(gatewaykit_admin_vars.ajax_error_occurred) + '</p>');
                }
            });
        },
        ...
        renderNotes: function(notes, transactionId) {
            ...
            html += '<h3>' + self.escapeHtml(gatewaykit_admin_vars.notes) + '</h3>';
            ...
            if (notes && notes.length > 0) {
                ...
                html += '<p>' + self.escapeHtml(gatewaykit_admin_vars.no_notes) + '</p>';
            }

            // Add note form
            html += '<div class="gatewaykit-add-note">';
            html += '<textarea id="gatewaykit-new-note" placeholder="' + self.escapeHtml(gatewaykit_admin_vars.add_note_placeholder) + '" rows="3"></textarea>';
            html += '<button type="button" class="button button-primary gatewaykit-save-note" data-transaction-id="' + transactionId + '">' + self.escapeHtml(gatewaykit_admin_vars.save_note) + '</button>';
            html += '</div>';
        ...
        showErrorDetailsModal: function(transactionId) {
            ...
            if (!$('#gatewaykit-error-details-modal').length) {
                var modalHtml = '<div id="gatewaykit-error-details-modal" class="gatewaykit-modal gatewaykit-error-modal" style="display: none;">' +
                    ...
                    '<h2>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error_details : 'Error Details') + '</h2>' +
                    ...
                    '<p>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.loading : 'Loading...') + '</p>' +
                    '</div>' +
                    '</div>' +
                    '</div>' +
                    '</div>';
            ...
            content.html('<p>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.loading : 'Loading...') + '</p>');
            modal.show();
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        html += '<h4>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error_summary : 'Error Summary') + '</h4>';
                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error_type : 'Type') + ':</strong> ' + self.escapeHtml(data.error_type_label || data.error_type || 'Unknown') + '</p>';
                        html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error_message : 'Message') + ':</strong> ' + self.escapeHtml(data.error_message || 'No message available') + '</p>';

                        if (data.error_code) {
                            html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error_code : 'Code') + ':</strong> ' + self.escapeHtml(data.error_code) + '</p>';
                        }

                        if (data.error_timestamp) {
                            html += '<p><strong>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.timestamp : 'Timestamp') + ':</strong> ' + self.escapeHtml(data.error_timestamp) + '</p>';
                        }

                        if (data.error_details && Object.keys(data.error_details).length > 0) {
                            html += '<h4>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error_details : 'Error Details') + '</h4>';
                            $.each(data.error_details, function(key, val) {
                                if (typeof val === 'object') {
                                    val = JSON.stringify(val, null, 2);
                                }
                                html += '<p><strong>' + self.escapeHtml(key) + ':</strong> ' + self.escapeHtml(val) + '</p>';
                            });
                        }

                        var textToCopy = 'Error Type: ' + (data.error_type_label || data.error_type || 'Unknown') + '\n' +
                            'Error Message: ' + (data.error_message || 'No message available') + '\n';
                        if (data.error_code) {
                            textToCopy += 'Error Code: ' + data.error_code + '\n';
                        }
                        if (data.error_timestamp) {
                            textToCopy += 'Timestamp: ' + data.error_timestamp + '\n';
                        }
                        if (data.error_details) {
                            textToCopy += 'Details: ' + JSON.stringify(data.error_details, null, 2) + '\n';
                        }

                        html += '<button class="gatewaykit-copy-button" data-copy-text="' + encodeURIComponent(textToCopy) + '">' +
                            self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.copy_error_details : 'Copy Error Details') +
                            '</button>';

                        html += '</div>';
                        content.html(html);
                    } else {
                        content.html('<p>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.error : 'Error') + ': ' + self.escapeHtml(response.data || (gatewaykit_admin_vars ? gatewaykit_admin_vars.unknown_error : 'Unknown error')) + '</p>');
                    }
                },
                error: function() {
                    content.html('<p>' + self.escapeHtml(gatewaykit_admin_vars ? gatewaykit_admin_vars.ajax_error_occurred : 'AJAX error occurred') + '</p>');
                }
            });
        }
    };
    ...
    $.extend(window.GatewayKit.Admin, AdminTransactions);

    $(document).ready(function() {
        AdminTransactions.init();
    });
