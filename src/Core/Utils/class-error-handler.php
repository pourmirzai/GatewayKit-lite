...
class GatewayKit_Error_Handler {
	...
	public function attempt_error_recovery($error_type, $error_code, $context = array()) {
		...
		$logger->info('Attempting error recovery', array(
			'error_type' => $error_type,
			'error_code' => $error_code,
		...
		if ($recovery_attempted) {
			$logger->info('Error recovery completed', array(
				'error_type' => $error_type,
				'error_code' => $error_code,
		...
		switch ($error_code) {
			...
			case 'verification_failed':
				...
				if (isset($context['transaction_id'])) {
					wp_schedule_single_event(time() + 300, 'gatewaykit_retry_payment_verification', array(
	...
	public function create_structured_error_response($error_type, $error_code, $context = array(), $is_admin = false) {
		...
		// Log the error
		$logger->error('Structured error created', array(
			'error_type' => $error_type,
			'error_code' => $error_code,
			'context' => $context,
			'is_admin' => $is_admin
