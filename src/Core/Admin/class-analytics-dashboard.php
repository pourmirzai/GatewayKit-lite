...
class GatewayKit_Analytics_Dashboard {
	...
	public function __construct() {
		...
		// (license-gated, ordered in the admin menu) — not via an admin_menu hook.
	...
	public function enqueue_assets( $hook_suffix ) {
		...
		wp_localize_script(
			...
			array(
				...
				'currency' => $this->get_configured_currency(),
				'i18n'     => array(
	...
	/**
	 * Get the configured payment currency (falls back to the build default).
	 *
	 * The admin can change the currency on the General settings tab, so the
	 * analytics panels must reflect that setting instead of hardcoding USD
	 * (ERR-042).
	 *
	 * @return string ISO 4217 code.
	 */
	private function get_configured_currency() {
		return strtoupper( get_option( 'gatewaykit_currency', GatewayKit_Gateway_Manager::get_instance()->get_default_currency() ) );
	}
	...
	public function render_page() {
		...
		<div class="wrap gatewaykit-analytics-wrap">
			<h1><?php /* translators: %s: Brand name. */ printf( esc_html__( '%s Analytics', 'gatewaykit' ), esc_html( $brand_name ) ); ?></h1>

			<div class="gatewaykit-analytics-filters">
				<label for="gk-period" class="screen-reader-text"><?php esc_html_e( 'Period', 'gatewaykit' ); ?></label>
				<select id="gk-period" name="gk-period">
				...
				<label for="gk-gateway" class="screen-reader-text"><?php esc_html_e( 'Gateway', 'gatewaykit' ); ?></label>
				<select id="gk-gateway" name="gk-gateway">
	...
	public function ajax_get_data() {
		...
		$cache_key = 'gk_analytics_' . md5( $days . '_' . $gateway . '_' . GATEWAYKIT_VERSION );
		...
		foreach ( $form_rows as $row ) {
			...
			if ( $post_id > 0 ) {
				// Post/page ID available — show the page title.
				$title = get_the_title( $post_id );
				if ( $title ) {
					$form_title = $title;
				} else {
					/* translators: %s: page ID */
					$form_title = sprintf( __( 'Page #%s (deleted)', 'gatewaykit' ), $post_id );
				}
			} elseif ( 'direct_ajax' === $form_id || '' === $form_id ) {
				// Direct AJAX payment or missing form info.
				$form_title = __( 'Direct Payment', 'gatewaykit' );
			} else {
				// form_id set but no post_id — try as a numeric post ID.
				if ( ctype_digit( $form_id ) ) {
					$guess_post = (int) $form_id;
					$title      = get_the_title( $guess_post );
					if ( $title ) {
						$form_title = $title;
					}
				}
				if ( '' === $form_title ) {
					...
					$form_title = sprintf( __( 'Form %s', 'gatewaykit' ), $form_id );
				}
			}
