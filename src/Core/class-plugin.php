...
class GatewayKit {
	...
	public function init_admin_components() {
		...
		// Analytics Dashboard: instantiate once here (registers the AJAX handler
		// on admin_init — the only hook that fires on admin-ajax.php) and reuse
		// the same instance in register_admin_menus() for the page callback.
	...
	public function register_admin_menus() {
		...
		if ( $pro_licensed && class_exists( 'GatewayKit_Analytics_Dashboard' ) ) {
			// Reuse the instance created by init_admin_components() so the
			// AJAX handler and enqueue hooks are registered exactly once.
			if ( ! $this->analytics_dashboard instanceof GatewayKit_Analytics_Dashboard ) {
				$this->analytics_dashboard = new GatewayKit_Analytics_Dashboard();
			}
			add_submenu_page(
				'gatewaykit',
				...
				array( $this->analytics_dashboard, 'render_page' )
			);
		} else {
			add_submenu_page(
				'gatewaykit',
		...
		// --- Transactions (registered separately to control ordering) — KEEP AS-IS ---
		if ( method_exists( $this->admin_settings, 'add_transactions_menu' ) ) {
			$this->admin_settings->add_transactions_menu();
		}
		...
		if ( $pro_licensed && class_exists( 'GatewayKit_Admin_Discounts' ) ) {
			...
			add_submenu_page(
				...
				array( $this->admin_discounts, 'render_page' )
			);
		} else {
			add_submenu_page(
				'gatewaykit',
		...
		// 6. Payment Links (registered directly to control ordering).
		if ( class_exists( 'GatewayKit_Payment_Links' ) ) {
			$pl = new GatewayKit_Payment_Links();
			$pl->add_menu();
		}

		// 7-9. Settings, Logs, Usage Guide.
		...
		// Note: White Label settings are accessible only via the Settings → White Label
		// tab (reachable via magic link when WL is active). No separate submenu item.
		// Note: Freemius "Account" and "Add-Ons" submenus are registered at
		// WP_FS__LOWEST_PRIORITY (9999) and naturally appear at the bottom.
	...
	private function process_payment_direct( $payment_data ) {
		...
		try {
			...
			if ( is_wp_error( $transaction ) ) {
				$logger->error(
					'Failed to create transaction for direct payment',
					array(
			...
			if ( ! $gateway ) {
				$logger->error(
					'Gateway not found for direct payment',
					array(
			...
			if ( ! isset( $enabled_gateways[ $payment_data['gateway'] ] ) || $enabled_gateways[ $payment_data['gateway'] ] !== '1' ) {
				$logger->error(
					'Gateway disabled for direct payment',
					array(
						'gateway' => $payment_data['gateway'],
					)
				);
			...
			if ( ! $gateway->is_available() ) {
				$logger->error(
					'Gateway unavailable for direct payment',
					array(
						'gateway' => $payment_data['gateway'],
					)
				);
			...
			if ( isset( $result['authority'] ) ) {
				...
				$logger->info(
					'Direct payment initiated successfully',
					array(
			...
			$logger->error(
				'Direct payment initiation failed',
				array(
					'transaction_id' => $transaction->get( 'id' ),
					'gateway'        => $payment_data['gateway'],
				)
			...
			$logger->error(
				'Exception during direct payment processing',
				array(
					'error' => $e->getMessage(),
					'trace' => $e->getTraceAsString(),
				)
